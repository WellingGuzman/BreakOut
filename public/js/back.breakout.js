    ...
    var BreakOut = {
        ...
        init: function(){
            ...
            for (i=0; i < BreakOut.bricks.rows; i++) {
                ...
                for (j=0; j < BreakOut.bricks.cols; j++) {
                    BreakOut.bricks.values[i][j] = true;//{ok: true, color:color};
                }
            }
        ...
        collide: function(){
            ...
            var row = Math.floor(BreakOut.ballPosition.y/h);
            var col = Math.floor(BreakOut.ballPosition.x/w);

            $("footer").prepend("<br>row:" + r + ", col: " + c);
            if ( BreakOut.ballPosition.y < BreakOut.ballWidth + (BreakOut.bricks.rows * BreakOut.bricks.h)
                    && BreakOut.bricks.values[row][col] ){
                //alert("colum:"+ r + " "+ c +" -> " + BreakOut.bricks.values[r]);
                BreakOut.bricks.values[row][col] = false;
                BreakOut.score += 10;
